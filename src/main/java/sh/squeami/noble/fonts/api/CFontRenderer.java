package sh.squeami.noble.fonts.api;

import net.minecraft.client.renderer.GlStateManager;
import net.minecraft.client.renderer.texture.DynamicTexture;
import net.minecraft.util.ResourceLocation;
import org.lwjgl.opengl.GL11;
import sh.squeami.noble.utils.font.FontUtil;
...
public class CFontRenderer extends CFont {
    ...
    public CFontRenderer(String NameFontTTF, int size, int fonttype, boolean antiAlias, boolean fractionalMetrics) {
        super(FontUtil.getFontFromTTF(new ResourceLocation("kami/font/" + NameFontTTF + ".ttf"), size, fonttype), antiAlias, fractionalMetrics);
        setupMinecraftColorcodes();
        setupBoldItalicIDs();
    }
    ...
    public float drawCenteredString(String text, double x, double y, int color) {
        return drawString(text, x - (double) getStringWidth(text) / 2, y, color);
    }

    public float drawCenteredStringWithShadow(String text, float x, float y, int color) {
        float shadowWidth = drawString(text, x - (double) getStringWidth(text) / 2 + 0.45D, y + 0.5D, color, true);
        return drawString(text, x - (float) getStringWidth(text) / 2, y, color);
    }
